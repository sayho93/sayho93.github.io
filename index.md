...
* 유니테이블은 국내 대학을 대상으로하는 자동 시간표 및 자동 과제관리와 더불어 자동 GPS기반 출결관리 앱

**개요**

* 전공 과목인 소프트웨어공학개론에서 진행된 프로젝트로서 진행된 본 앱은 애자일 프로세스를 적용하여 수행됨.
* 현재 운용되는 모든 대학 시간표 앱들은 모두 자신의 수업을 수기로 입력하거나 수업을 일일히 선택하여 배치해야 하며, 과제의 경우, 대학에서 운영하는 웹사이트에 접속하여 과목을 선택하여 봐야하는 접근성 측면에서의 큰 불편을 가짐.
* 그리하여, 현재의 초기버전으로서 '동국대학교', '서강대학교', '국민대학교(테스트)'에 대해 서비스를 제공 중.
* 학생들에게 가장 중요한 출결을 GPS를 기반으로 자동으로 체크하여 푸시알림을 통해 알리고 기록하도록 구성하여 편리함을 더함.
* 모바일에서의 **접근성**을 증대시키고, 홈 위젯을 통해 보다 빠르게 시간표를 확인할 수 있도록 구성.
* 학사 정보를 통한 회원가입이 필요없는 로그인 서비스를 제공.
* 커뮤니티 및 회원간 채팅 기능

**특징**
* 회원가입이 전혀 필요하지 않으며, 학사 정보로 로그인하여 모든 정보를 별도의 조작없이 확인.
* 별도의 조작없이 모든 시간표와 과제 목록을 보여줌.
* GPS를 기반으로 사용자가 수업시간에 학교에 있었는지 자동으로 기기 내에 기록.

* 본 앱의 시초가 된 [동국헬퍼](https://play.google.com/store/apps/details?id=com.planner.dgu.dguplan)는 동국대학교 학생 약 400여 명이 이용중. (2016/12/05 기준)
